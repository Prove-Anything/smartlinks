...
export const SHARED_DEPENDENCY_CONTRACT_VERSION = 'v7';
/**
...
export const SHARED_DEPENDENCIES = CONTRACT.map((d) => (Object.assign(Object.assign({}, d), { importMapPath: importMapPathFor(d.specifier) })));
/** Just the bare specifiers — handy for a bundler `external` list. */
export const SHARED_DEPENDENCY_SPECIFIERS = SHARED_DEPENDENCIES.map((d) => d.specifier);
/**
 ...
 */
export function getHostSharedDependencies() {
    const g = globalThis;
    ...
    const specifiers = Array.isArray(marker.specifiers)
        ? marker.specifiers.filter((s) => typeof s === 'string')
        : null;
    if (!version || !specifiers)
        return null;
