...
(function (authKit) {
     ...
     * Gated by step-up MFA — see {@link login} for the `MFA_REQUIRED` error shape.
     */
    async function linkedInLogin(clientId, code, redirectUri, trustedDeviceToken) {
        const res = await post(`/authkit/${encodeURIComponent(clientId)}/auth/linkedin`, { code, redirectUri, trustedDeviceToken });
        if (res.token) {
            setBearerToken(res.token);
            invalidateCache();
        }
    ...
    authKit.linkedInLogin = linkedInLogin;
    /**
