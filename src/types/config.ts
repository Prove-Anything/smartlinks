...
export type FieldType =
  | 'text'          // single-line text input (also handles email and url sub-types)
  | 'email'         // single-line, type="email" validation
  | 'url'           // single-line, type="url" validation
  | 'textarea'      // multi-line text
  | 'number'        // numeric — supports min / max / step
  | 'select'        // dropdown — requires options[]; supports multiple + chips
  | 'switch'        // boolean toggle
  | 'checkbox'      // boolean checkbox
  | 'date'          // date picker (HTML date input)
  | 'json'          // raw JSON editor with parse validation
  | 'image-upload'  // file picker → auto-uploads to Assets API → stores CDN URL string; shows thumbnail
  | 'file-upload'   // file picker → auto-uploads to Assets API → stores CDN URL string
