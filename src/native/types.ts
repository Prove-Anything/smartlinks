...
export interface NfcFacade {
   ...
   */
  read(opts?: { timeoutMs?: number }): Promise<NfcReadResult>;
  /** Write an NDEF record to a tag by UID. */
  writeNdef(opts: { uid: string; payload: string }): Promise<void>;
  /** Program an NTAG 21x / 424 profile. Host-specific; Kotlin only on advanced ops. */
  programTag(opts: { uid: string; profile: string; [key: string]: unknown }): Promise<void>;
  /** Lock a tag against further writes. */
  lockTag(opts: { uid: string }): Promise<void>;
  /** Returns whether the tag is currently locked. */
  isLocked(opts: { uid: string }): Promise<boolean>;
}
